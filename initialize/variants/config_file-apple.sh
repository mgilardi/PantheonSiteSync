...
function process_config_file_variables() {
    ...
    if [ -z "$DRUPAL_GIT_CLONE" ]; then
        ...
        DRUPAL_ROOT=$(gfind "$PROJECT_ROOT" -type l)
        ENV=$(echo "$REMOTE_PANTHEON_DASHBOARD_URL_RAW" | ggrep -oP '#\w+');
        ENV=${ENV:1}
    ...
    if [ -e "$DRUPAL_DEFAULT_DIR/settings.local.php" ]; then
      LOCAL_DB_PSWD=$(ggrep -oP "['\"]password['\"]\s*=>\s*['\"].*['\"]" "$DRUPAL_DEFAULT_DIR/settings.local.php" |cut -d"'" -f4)
    else
    ...
    LOCAL_DB_DB=$(echo "$LOCAL_DOMAIN_BASE" | tr '.-' '_' | cut -c 1-${len}|gsed 's/_$//')_${ENV}
    LOCAL_DB_USER=$LOCAL_DB_DB
    echo "  Local dtbase: $LOCAL_DB_DB"

    # For "mysql_config_editor"
    REMOTE_DB_LOGIN_PATH=$(echo "$LOCAL_DB_DB" | sed -e 's%[_-]%%g')
}
