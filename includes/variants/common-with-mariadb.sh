...
# function_echo_title
  ...
  local result=($(mysql -u "$REMOTE_DB_USER" -p"$REMOTE_DB_PSWD" -h "$REMOTE_DB_HOST" -P "$REMOTE_DB_PORT" "$REMOTE_DB_DB" -e "SHOW TABLES LIKE '%$test_table';"))
...
function set_LOCAL_DB_PREFIX() {
  ...
  if [ -z "$LOCAL_DB_PREFIX" ]; then
    if [ ! -f "$settings_file" ]; then
      ...
      LOCAL_DB_PREFIX=$(grep -P '["'"'"']prefix["'"'"']\s+=>\s+["'"'"'].+["'"'"']' "$settings_file"|grep -Pv '\s*[*/#]+'|cut -d'>' -f2|grep -oP '\w+')
    fi
  fi
}
...
function wake_up_server() {
  ...
  if [ -e "$persist" ]; then
    local last_accessed=$(stat -c %Y "$persist")
...
function waiting_for_process_to_start() {
  ...
  WAITED=0
  if [ $(ps aux|grep "$process"|grep -Pv '(grep|spawn)'|wc -l) -eq 0 ]; then
    echo "  Waiting for up to '$wait_to_start_time_max' seconds for subprocess '$process' to start."
    while [ $(ps aux|grep "$process"|grep -Pv '(grep|spawn)'|wc -l) -eq 0 ]; do
      sleep 1
      ((WAITED++))
...
function waiting_for_process_to_end() {
  ...
  WAITED=0
  while [ $(ps aux|grep "$process"|grep -Pv '(grep|spawn)'|wc -l) -gt 0 ]; do
    sleep 1
    ((WAITED++))
  done
