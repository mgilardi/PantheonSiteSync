...
#   OPTIND=1                # Not needed with "local OPTIND"
    ...
    if [ -z "$pswd" ]; then
        local pswd=$DRUPAL_ADMIN_USER_PSWD
        if [ -z "$pswd" ]; then
    ...
    if [ -z "$name" ]; then
        local name=$DRUPAL_ADMIN_USER_NAME
        if [ -z "$name" ]; then
            echo "User name missing. Please set optional parameter '-n'"
            exit
        fi
    fi
    ...
    local result=($(mysql --login-path="$login" "$db" -e "SELECT name,pass FROM $ut WHERE uid=1;"))
    ...
    local token=($("$phf" --root "$DRUPAL_ROOT" "$pswd"|cut -d':' -f3))
    ...
    #
    mysql --login-path="$login" "$db" -e "UPDATE $ut SET name='$name', pass='$token_esc' WHERE uid = 1;"
    echo
    echo "  Login-path:$login DB:$db user 1 set to:"
    echo "    name='$name'"
    echo "    pass='$token'"
    echo
}
...
function set_remote_user_one_pass() {
    ...
    set_remote_db_in_mysql_config_editor
    wake_up_server $REMOTE_URL $PANTHEON_KEEP_ALIVE
    set_user_one_password_token -d "$REMOTE_DB_DB" -l "$REMOTE_DB_LOGIN_PATH"
}
...
function set_local_user_one_pass() {
    set_user_one_password_token -d "$LOCAL_DB_DB" -l "$LOCAL_DB_LOGIN_PATH"
}
