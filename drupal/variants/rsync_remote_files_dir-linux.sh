...
function running_rsync_processes() {
  RSYNC_PROCESSES=$(ps aux|grep "sudo rsync"|grep -v 'grep'|wc -l|grep -oP '\w+')
}
...
function drupal_get_files_dir() {
    ...
    if [ $(find $TMP_DIR/ -maxdepth 1 -type f -name "rsync-*" | wc -l|grep -oP '\w+') -gt 0 ]; then # just in case the script was interrupted.
      rm -f $TMP_DIR/rsync-*
    fi
    ...
    sed -i "1d" "$TMP_DIR/$batch_file"
    split -a 3 -l $RSYNC_BATCH_SIZE "$TMP_DIR/$batch_file" "$TMP_DIR/$batch_file-"
    local files=($(find $TMP_DIR/ -name "$batch_file-*"))
    local file
